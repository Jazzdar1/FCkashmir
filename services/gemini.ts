...
// Helper to handle Quota errors
const handleQuotaError = (error: any, fallbackValue: any) => {
  const msg = error.message || '';
  if (error.status === 429 || msg.includes('429') || msg.toLowerCase().includes('quota') || msg.includes('RESOURCE_EXHAUSTED')) {
    console.warn("Gemini API Quota Exceeded. Returning fallback.");
...
export const analyzeCropDisease = async (base64Image: string, language: 'en' | 'ur' | 'hi' = 'en'): Promise<DiseaseAnalysis> => {
  try {
    ...
    return handleQuotaError(error, {
      diseaseName: "Service Busy (Quota Limit)",
      severity: "Low",
      confidence: 0,
      description: "The AI diagnostic service is currently unavailable due to high traffic/quota limits. Please try again later.",
      treatment: ["Retry in a few minutes"],
      preventiveMeasures: []
    });
  }
};
...
export const getDeepExpertView = async (base64Image: string, diseaseName: string): Promise<string> => {
  try {
    ...
    return handleQuotaError(error, "Expert analysis unavailable due to high traffic (Quota Exceeded).");
  }
};
...
export const generateUrduDiagnosisAudio = async (analysis: DiseaseAnalysis) => {
  try {
    ...
    return handleQuotaError(error, null);
  }
};
...
export const getExpertAdvice = async (history: { role: string; parts: { text: string }[] }[], prompt: string, language: 'en' | 'ur' | 'hi' = 'en') => {
  try {
    ...
    return handleQuotaError(error, "I am currently receiving too many requests (Quota Limit). Please try again in a few minutes.");
  }
};
...
export const getDistrictWeather = async (district: string) => {
  try {
    ...
    return handleQuotaError(error, {
      ...
      humidity: "--",
      windSpeed: "--",
      forecast: "Weather data unavailable due to high traffic (Quota Limit).",
      farmerTip: "Unable to load tip.",
      urduSummary: "سروس دستیاب نہیں ہے"
    });
  }
};
...
export const findNearbyMandis = async (lat: number, lng: number) => {
  try {
    ...
    return handleQuotaError(error, { text: "Search unavailable due to high traffic (Quota Limit).", sources: [] });
  }
};
...
export const analyzeLivestockHealth = async (base64Image: string, animalType: string) => {
  try {
    ...
    return handleQuotaError(error, {
      condition: "Service Busy",
      severity: "Unknown",
      advice: "API Quota exceeded. Please try again later.",
      urduSummary: "سروس مصروف ہے"
    });
  }
};
