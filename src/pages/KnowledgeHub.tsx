...
const categories = [
  ...
  { id: 'saved', label: 'Saved Offline', icon: Bookmark, color: 'amber' },
...
const varieties = [
  { id: 'v1', category: 'varieties', title: 'Kashmiri Delicious Apple', desc: 'The valley standard. Known for intense aroma and red stripes.', tags: ['Premium', 'Export'], status: 'Active' },
  { id: 'v2', category: 'varieties', title: 'Pampore Mongra Saffron', desc: 'Superior Grade A1. Highest crocin content worldwide.', tags: ['G-I Tag', 'Rare'], status: 'Harvesting' },
  { id: 'v3', category: 'varieties', title: 'Paper Shell Walnut (Kagzi)', desc: 'Thin shell, high oil content favorite from South Kashmir.', tags: ['Nut', 'Export'], status: 'Stable' },
  { id: 'v4', category: 'varieties', title: 'American Mother Apple', desc: 'Hardy variety, durable during mountain transit.', tags: ['Durable', 'Regional'], status: 'Active' },
  { id: 'v5', category: 'varieties', title: 'Mishri Heart Cherry', desc: 'Sweet heart-shaped cherries. Harvested in early summer.', tags: ['Delicate', 'Summer'], status: 'Seasonal' },
  { id: 'v6', category: 'varieties', title: 'Kashmiri Gurbandi Almonds', desc: 'Rich in oils, grown in Pulwama and Budgam.', tags: ['Nut', 'Oily'], status: 'Stable' },
  { id: 'v7', category: 'varieties', title: 'Ambri Apple (Legacy)', desc: 'Heritage variety, uniquely aromatic and long-storing.', tags: ['Heritage', 'Local'], status: 'Limited' },
  { id: 'v8', category: 'varieties', title: 'Conference Pear', desc: 'High yielding, adapted to high altitude. Good for jams.', tags: ['Hardy', 'Process'], status: 'Stable' },
];
...
const schemes = [
  { id: 's1', category: 'schemes', title: 'PM-Kisan J&K Installment', desc: 'Direct income support for small and marginal farmers.', tags: ['Financial', 'Income'], status: 'Active' },
  { id: 's2', category: 'schemes', title: 'High Density Plantation Subsidy', desc: '50% subsidy for project costs on high-density orchards.', tags: ['State', 'Subsidy'], status: 'Open' }
];

const guides = [
  { id: 'g1', category: 'guides', title: 'Official Spray Calendar', desc: 'SKUAST-K 2024-25 horticultural spray schedule.', tags: ['Manual', 'Calendar'], status: 'Current' },
  { id: 'g2', category: 'guides', title: 'Cold Storage (CA) Manual', desc: 'Guide to pre-cooling and long-term storage of apples.', tags: ['Storage', 'Guide'], status: 'Active' }
];

const tools = [
  { id: 't1', category: 'tools', title: 'Profit Estimator', desc: 'Calculator for estimating seasonal revenue based on Mandi rates.', tags: ['Calc', 'Finance'], status: 'Live' },
  { id: 't2', category: 'tools', title: 'Dosage Pro', desc: 'Precise chemical-to-water ratio calculator for orchard sprays.', tags: ['Mix', 'Safety'], status: 'Active' }
];
...
const KnowledgeHub: React.FC = () => {
  ...
  const [savedIds, setSavedIds] = useState<string[]>(() => {
    const saved = localStorage.getItem('fck_offline_articles');
    return saved ? JSON.parse(saved) : [];
  });
  const location = useLocation();

  useEffect(() => {
    localStorage.setItem('fck_offline_articles', JSON.stringify(savedIds));
  }, [savedIds]);
  ...
  const filtered = useMemo(() => {
    return knowledgeData.filter(item => {
      ...
      const matchesCategory = activeCat === 'all' || 
                             (activeCat === 'saved' && savedIds.includes(item.id)) ||
                             item.category === activeCat;
      
      return matchesSearch && matchesCategory;
    });
  }, [activeCat, searchTerm, savedIds]);
        ...
        <div className="flex gap-2 overflow-x-auto pb-4 no-scrollbar scroll-smooth">
          {categories.map((cat) => (
            ...
            >
              ...
              {cat.label}
              {cat.id === 'saved' && savedIds.length > 0 && (
                <span className="bg-amber-400 text-amber-900 px-2 py-0.5 rounded-full text-[10px] font-black">{savedIds.length}</span>
              )}
            </button>
          ))}
        </div>
          ...
          <div className="relative bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 max-h-[90vh] flex flex-col">
            <div className="bg-emerald-900 p-8 text-white flex justify-between items-start shrink-0">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  ...
                  {isSaved(selectedItem.id) && <span className="bg-amber-400/20 text-amber-300 border border-amber-400/30 px-2 py-0.5 rounded-md text-[8px] font-black uppercase">Offline Safe</span>}
                </div>
              ...
              <div className="flex items-center gap-2">
                <button 
                  ...
                  className={`p-3 rounded-full transition-all border ${isSaved(selectedItem.id) ? 'bg-amber-400 border-amber-500 text-amber-900 shadow-lg' : 'bg-white/10 border-white/20 text-white hover:bg-white/20'}`}
                >
                  {isSaved(selectedItem.id) ? <BookmarkCheck className="w-6 h-6" /> : <Bookmark className="w-6 h-6" />}
                </button>
            ...
            <div className="p-8 lg:p-12 overflow-y-auto no-scrollbar flex-1 space-y-10">
              ...
              <div className="flex flex-col gap-3">
                <button 
                  onClick={(e) => toggleSave(selectedItem.id, e)}
                  className={`w-full py-5 rounded-2xl font-bold shadow-xl transition-all flex items-center justify-center gap-2 ${isSaved(selectedItem.id) ? 'bg-rose-50 text-rose-600 border border-rose-100 hover:bg-rose-100' : 'bg-emerald-600 text-white hover:bg-emerald-700'}`}
                >
                  {isSaved(selectedItem.id) ? (
                    <><X className="w-5 h-5" /> Remove from Offline</>
                  ) : (
                    <><Download className="w-5 h-5" /> Save for Offline Access</>
                  )}
                </button>
                <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filtered.length > 0 ? (
          filtered.map((item, idx) => {
            ...
            return (
              <div 
                ...
                className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm hover:shadow-2xl hover:border-emerald-200 transition-all cursor-pointer group flex flex-col justify-between relative overflow-hidden"
              >
                {saved && (
                  <div className="absolute top-0 right-0 p-4">
                    <BookmarkCheck className="w-6 h-6 text-amber-500 fill-amber-500/10" />
                  </div>
                )}
                ...
                <div className="mt-8 pt-6 border-t border-slate-50 flex items-center justify-between">
                  ...
                  <div className="text-xs font-black uppercase tracking-widest text-emerald-700 flex items-center gap-1 group-hover:translate-x-1 transition-transform">
                    {saved ? 'Open Saved' : 'Explore'} <ChevronRight className="w-3.5 h-3.5" />
                  </div>
                </div>
              </div>
            );
          ...
          <div className="col-span-full py-20 text-center space-y-4">
             ...
             {activeCat === 'saved' && (
               <p className="text-slate-400 text-sm font-medium">You haven't saved any articles for offline access yet.</p>
             )}
          </div>
        )}
      </div>
